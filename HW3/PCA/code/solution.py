...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        #initial_weights = nn.init.kaiming_normal_(weights)
        #self.weights = nn.Parameter(init_weights)
        #self.w = self.weights

        # Note: here for the network without weights sharing 
        input_weights = torch.empty(self.n_features, self.d_hidden_rep)
        initial_input_weights = nn.init.kaiming_normal_(input_weights)
        self.input_weights = nn.Parameter(initial_input_weights)
        self.w = self.input_weights

        output_weights = torch.empty(self.d_hidden_rep, self.n_features)
        initial_output_weights = nn.init.kaiming_normal_(output_weights)
        self.output_weights = nn.Parameter(initial_output_weights)

        # Note: here for the network with more layers and nonlinear functions  
        #scaling = 2

        #input_weights_1 = torch.empty(self.n_features, scaling * self.d_hidden_rep)
        #initial_input_weights_1 = nn.init.kaiming_normal_(input_weights_1)
        #self.input_weights_1 = nn.Parameter(initial_input_weights_1)

        #input_weights_2 = torch.empty(scaling * self.d_hidden_rep, self.d_hidden_rep)
        #initial_input_weights_2 = nn.init.kaiming_normal_(input_weights_2)
        #self.input_weights_2 = nn.Parameter(initial_input_weights_2)
        #self.w = self.input_weights_2

        #output_weights_2 = torch.empty(self.d_hidden_rep, scaling * self.d_hidden_rep)
        #initial_output_weights_2 = nn.init.kaiming_normal_(output_weights_2)
        #self.output_weights_2 = nn.Parameter(initial_output_weights_2)

        #output_weights_1 = torch.empty(scaling * self.d_hidden_rep, self.n_features)
        #initial_output_weights_1 = nn.init.kaiming_normal_(output_weights_1)
        #self.output_weights_1 = nn.Parameter(initial_output_weights_1)
    ...
    def _forward(self, X):
        ...
        #out = torch.mm(self.weights, hidden)
        #return out


        # Note: here for the network without weights sharing 
        hidden = torch.mm(self.input_weights.t(), X)
        output = torch.mm(self.output_weights.t(), hidden)
        return output

        # Note: here for the network with more layers and nonlinear functions
        #in1 = torch.relu(torch.mm(self.input_weights_1.t(), X))
        #in2 = torch.relu(torch.mm(self.input_weights_2.t(), in1))
        #out2 = torch.relu(torch.mm(self.output_weights_2.t(), in2))
        #out1 = torch.sigmoid(torch.mm(self.output_weights_1.t(), out2))
