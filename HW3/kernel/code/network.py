...
class Kernel_Layer(nn.Module):
    ...
    def _k_means(self, X):
        ...
        centroids = x[indices,:]
        old = torch.randint(0,self.hidden_dim,(X.shape[0],),dtype=torch.long)
        ### END YOUR CODE
        return centroids
    ...
    def forward(self, x):
        ...
        n = (x.shape[0], self.prototypes.shape[0], x.shape[1])
        x = x.unsqueeze(1).expand(n)
        p = self.prototypes.unsqueeze(0).expand(n)
        norm = ((x - p)**2).sum(-1).pow(1/2)
...
class Kernel_LR(nn.Module):
    ...
    def __init__(self, sigma, hidden_dim):
        ...
        self.net = nn.Sequential(Kernel_Layer(sigma),nn.Linear(hidden_dim, 1, False))
        ### END YOUR CODE
...
class FFN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim):
        ...
        self.net = nn.Sequential(nn.Linear(input_dim, hidden_dim, True),nn.Linear(hidden_dim, 1, True))
        ### END CODE HERE
