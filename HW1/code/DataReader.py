...
def prepare_X(raw_X):
	...
	### YOUR CODE HERE
    flip = np.flip(raw_image, axis = -1)
    symmetry = -np.sum(np.abs(raw_image - flip), axis = (-1,1))/256
	### END YOUR CODE

	# Feature 2: Measure of Intensity
	### YOUR CODE HERE
    intensity = np.sum(raw_image, axis = (-1,1))/256	
	### END YOUR CODE
	...
	### YOUR CODE HERE
    X = np.vstack([bias, symmetry, intensity]).transpose()
	### END YOUR CODE
    return X
