...
class Config(object):
  ...
  max_epochs = 250 # the number of max epoch
  early_stopping = 3
  dropout = 0.10
  lr = 0.001
  momentum = 0.0
...
class RNNLM_Model(nn.Module):
  ...
  def add_model(self, input_x, initial_state):
    ...
    for inx in input_x:
      a = torch.matmul(final_state,self.H)
...
def compute_loss(outputs, y, criterion):
  ...
  """ 
  ### YOUR CODE HERE
  #had to correct starter function, because it had typo of ouputs rather than outputs.
