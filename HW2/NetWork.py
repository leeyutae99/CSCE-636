...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.batch_n1 = nn.BatchNorm2d(num_features=filters, eps=1e-5, momentum=0.997)
        self.relu1 = nn.ReLU()

        self.conv2 = nn.Conv2d(in_channels=filters, out_channels=filters, stride=1, padding=1, kernel_size=3)
        self.batch_n2 = nn.BatchNorm2d(num_features=filters, eps=1e-5, momentum=0.997)
        self.relu2 = nn.ReLU()
        ### YOUR CODE HERE
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        outputs = self.batch_n1(outputs)
        outputs = self.relu1(outputs)
        # second convolution layer
        outputs = self.conv2(outputs)
        outputs = self.batch_n2(outputs)
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.batch_n1 = nn.BatchNorm2d(num_features=filters, eps=1e-5, momentum=0.997)
        self.relu1 = nn.ReLU()
        self.conv1 = nn.Conv2d(in_channels=filters, out_channels=filters//4, stride=1, padding=0, kernel_size=1)
        self.batch_n2 = nn.BatchNorm2d(num_features=filters//4, eps=1e-5, momentum=0.997)
        self.relu2 = nn.ReLU()
        self.conv2 = nn.Conv2d(in_channels=filters//4, out_channels=filters//4, stride=1, padding=1, kernel_size=3)
        self.batch_n3 = nn.BatchNorm2d(num_features=filters//4, eps=1e-5, momentum=0.997)
        self.relu3 = nn.ReLU()
        self.conv3 = nn.Conv2d(in_channels=filters//4, out_channels=filters, stride=1, padding=1, kernel_size=3)
        ### YOUR CODE HERE
...
class stack_layer(nn.Module):
    ...
    def __init__(self, filters, block_fn, strides, resnet_size, first_num_filters) -> None:
        ...
        if block_fn is standard_block:
            ...
            filters_in = filters if first_num_filters == filters else filters * 2
            self.projection_shortcut = nn.Sequential(
                nn.Conv2d(in_channels=filters_in, out_channels=filters_out, stride=strides, kernel_size=1,
                          bias=False),
                nn.BatchNorm2d(filters_out)
            )
        stack_layers = []
