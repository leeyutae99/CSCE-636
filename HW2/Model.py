    ...
    def train(self, x_train, y_train, max_epoch):
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for i in range(num_batches):
                ...
                x_batch = [parse_record(x, True) for x in curr_x_train[i * self.config.batch_size: (i + 1) * self.config.batch_size]]
                ...
                loss = self.crossEntropyloss(outputs, y_batch_tensor.long())
                ### YOUR CODE HERE
    ...
    def test_or_validate(self, x, y, checkpoint_num_list):
        ...
        for checkpoint_num in checkpoint_num_list:
            ...
            for i in tqdm(range(x.shape[0])):
                ### YOUR CODE HERE
                current_x = torch.tensor(parse_record(x[i], False)).float().cuda()
                output = self.network(current_x.view(1, 3, 32, 32))
                predict = int(torch.max(output.data, 1)[1])
